...
public class Main {
	public static void main(String[] args) {
		...
		do {
			//CADASTRO DO PRODUTO
			System.out.println("#############################################");
			System.out.printf("CADASTRANDO PRODUTO Nº %d:\n", cont);
			...
			char decisao;
			while(true) {
				System.out.print("Produto está na promoção? [S/N]:");
				decisao = scan.next().toUpperCase().charAt(0);
				if((decisao != 'S') && (decisao != 'N')){
					System.out.print("ERRO, INSIRA UM CARACTER VÁLIDO! ");
				}
				else {
					break;
				}
			...
			System.out.println("-=--=--=--=--=--=--=--=--=--=--=--=--=--=--=-");
			System.out.printf("CADASTRANDO VENDEDOR Nº %d:\n", cont);
			...
			Vendedor vendedor = new Vendedor(nome, endereco, codigoVendedor);
			...
			System.out.println("-=--=--=--=--=--=--=--=--=--=--=--=--=--=--=-");
			while(true) {
				System.out.print("Você deseja realizar um novo cadastro? [S/N]:");
